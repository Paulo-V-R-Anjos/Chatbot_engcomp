...
def space_correction(text):
  return re.sub(r"([a-zA-Z])([A-Z])", r"\1 \2", text)
...
texts = [space_correction(chunk) for doc in all_documents for chunk in text_splitter.split_text(doc.page_content)]
...
ollama_model = OllamaLLM(model="llama3.2", max_tokens=2000)

# Criando a Cadeia de Perguntas e Respostas com prompt personalizado
# qa_chain = RetrievalQA.from_chain_type(
#    llm=ollama_model,
#    chain_type="stuff",
#    retriever=docsearch.as_retriever(search_kwargs={"k": 7}), # k=3
